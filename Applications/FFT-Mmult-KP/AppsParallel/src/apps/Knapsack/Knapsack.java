...
public final class Knapsack extends Runner implements KnapsackInterface, java.io.Serializable {
	 ...
	 * 
	 */
	private static final long serialVersionUID = -5909662657106706424L;
	...
	public Return spawn_try_it(int i, int tw, int av, int limw, int maxv, int[] values, int[] weights, byte[] s,
			byte[] opts) {
		return try_it(i, tw, av, limw, maxv, values, weights, s, opts);
	}

	public Return try_it(int i, int tw, int av, int limw, int maxv, int[] values, int[] weights, byte[] s,
			byte[] opts) {
		int k, av1, maxv2;
		byte[] optdup = null;
		av1 = av - values[i];
	...
	/*
	 * System.out.println("Table is now:"); for (int o=0;o<N+1; o++) {
	 * System.out.println("index " + o + " weight " + weights[o] + " value " +
	 * values[o] + " prio " + prios[o]); }
	 */
