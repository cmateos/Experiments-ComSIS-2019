...
RevisionHandler {
  ...
  protected Instance[] m_Instances; 
  ...
  private int capacity, pos = 0;
  private static final int DEFAULT_CAPACITY = 24000;
...
*/
  ...
  protected void initialize(Instances dataset, int capacity) {
    ...
    m_Instances = new Instance[capacity];
  }
  ...
  public Instances(/* @non_null@ */String name,
    ...
    this.capacity = capacity;
    m_Instances = new Instance[capacity];//new ArrayList<Instance>(capacity);
  }
...
System.out.println("Se llama");
	  ArrayList<Instance> instances = (ArrayList<Instance>)Arrays.asList(m_Instances);
    //m_Instances.trimToSize();
	  instances.trimToSize();
	  m_Instances = (Instance[]) instances.toArray();
  }
  ...
  public void delete() {
    ...
    m_Instances = null; //new Instance[DEFAULT_CAPACITY];//new ArrayList<Instance>();
  }
...
System.out.println("Se llama delete!");	  
...
//	  overrideElem(index);
  ...
  public void overrideElem(int index){
	  for (int i = index; i < pos-1; i++){
		  m_Instances[i] = m_Instances[i+1];
	  }
  }
...
int pointer=0;
	...
	m_Instances[pointer]=instance(i);
	pointer++;
      }
    }
    //m_Instances = newInstances;
    //m_Instances = (Instance[]) newInstances.toArray(m_Instances);
	pos=pointer;
  }
...
/*	  List<Instance> array = new ArrayList<>(pos);
	  for (int i = 0; i < pos; i++)
		  array.add(i, m_Instances[i]);*/
	  
//    return new MyWekaEnumeration<Instance>(array);
    return new MyWekaEnumeration<Instance>(m_Instances, pos);
  }
  ...
  public/* @non_null pure@ */Instance firstInstance() {
    ...
    return m_Instances[0];//m_Instances.get(0);
  }
  ...
  public/* @non_null pure@ */Instance instance(int index) {
    ...
    return m_Instances[index];//m_Instances.get(index);
  }
  ...
  public/* @non_null pure@ */Instance lastInstance() {
    ...
    return m_Instances[pos-1];//m_Instances.get(m_Instances.size() - 1);
  }
  ...
  public/* @pure@ */int size() {
	return pos;
    //return m_Instances.length;//.size();
  }
...
System.out.println("Se llama remove index1!!!");
	  Instance instance = m_Instances[index];
	  overrideElem(index);
    return instance;//m_Instances.remove(index);
  }
...
System.out.println("Se llama set!!!");
    Instance newInstance = (Instance) instance.copy();
    Instance oldInstance = m_Instances[index];//m_Instances.get(index);
  ...
  protected void sortBasedOnNominalAttribute(int attIndex) {
    ...
    for (Instance inst : backup) { // Use backup here
      if (!inst.isMissing(attIndex)) {
    	  m_Instances[indices[(int)inst.value(attIndex)]++] = inst;
        //m_Instances.set(indices[(int)inst.value(attIndex)]++, inst);
      } else {
    	  m_Instances[start++] = inst;
  ...
  public void sort(int attIndex) {
    ...
    if (!attribute(attIndex).isNominal()) {
      ...
      for (int i = 0; i < vals.length; i++) {
        ...
        m_Instances[i] = backup[sortOrder[i]];
      }
    } else {
...
System.out.println("stratStep");

    ArrayList<Instance> newVec = new ArrayList<Instance>(m_Instances.length);//m_Instances.size());
    int start = 0, j;
    ...
    m_Instances = (Instance[])newVec.toArray();
  }
  ...
  public void swap(int i, int j) {
    ...
    Instance in = m_Instances[i];//m_Instances.get(i);
    m_Instances[i] = m_Instances[j];//m_Instances.set(i, m_Instances.get(j));
    m_Instances[j] = in;//m_Instances.set(j, in);
  }
